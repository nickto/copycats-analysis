with 
  ...
  stock_data_extra as (
    ...
    select
      ...
      lag(s.date) over (partition by s.ncusip order by s.date) as lag_stock_date,
      s.prc,
      lag(s.prc) over (partition by s.ncusip order by s.date) as lag_prc,
      --s.prc / s.cfacpr as pr_adj,
      --lag(s.prc / s.cfacpr) over (partition by s.ncusip order by s.date) as lag_prc_adj,
      s.cfacshr,
      lag(s.cfacshr) over (partition by s.ncusip order by s.date) as lag_cfacshr,
      s.ret,
      s.exchcd,
      s.shrout,
