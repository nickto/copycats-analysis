...
| **Step 2: Apply your company codes.** | Select an option for entering company codes | Upload a `cusip-list.txt` file that results from running `03-export-cusips.R` |
|  | Conditional Statements (Optional) | None (leave fields empty) |
| **Step 2: Query Variables.** | Select | All |
| **Step 3: Select query output.** | Output Format | `comma-delimited text (*.csv) ` |
| | Compression Type | `gzip (*.gz)` |
...
The output is saved into `~/data/` directory under the name `wrds-crsp-stocks-19900101-20151231.csv.gz`.
...
| Key | Value |
| ------------- | ------------- |
| Data Request ID | c15756b0ac0282d7 |
